...
using namespace std;
int Hash::FuncaoHash(Aluno aluno) { return (aluno.obterRa() % max_posicoes); }
...
bool Hash::estacheio() { return (quant_itens == max_itens); }

int Hash::obterTamanhoAtual() { return (quant_itens); }

void Hash::inserir(Aluno aluno) {
  if (estacheio()) {
    cout << "A Tabela hash esta cheio!\n";
    cout << "O elemento não pode ser insirido!\n";
  } else {
